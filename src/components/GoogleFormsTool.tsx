    ...
    <div className="flex flex-col gap-4 text-xs">
      ...
      <div className="flex items-center justify-between p-3 rounded-2xl bg-purple-950/40 border border-purple-500/20">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="font-semibold text-purple-200">Google Forms API Active</div>
        ...
        {!accessToken && (
          <button
            ...
            className="px-3 py-1.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-medium transition-all text-[11px]"
          >
            Authorize Workspace
          </button>
        )}
          ...
          <div className="space-y-3 bg-zinc-900/60 p-4 rounded-2xl border border-white/5">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-xl bg-black/50 border border-white/10 text-zinc-100 focus:outline-none focus:border-purple-500"
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                className="w-full px-3 py-2 rounded-xl bg-black/50 border border-white/10 text-zinc-100 focus:outline-none focus:border-purple-500 resize-none"
            ...
            <div className="space-y-3 pt-2">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="flex items-center gap-1 text-purple-400 hover:text-purple-300 font-medium text-[11px]"
                >
                  <Plus className="w-3.5 h-3.5" />
                  Add Question
                </button>
              ...
              {questions.map((q, idx) => (
                <div key={q.id} className="p-3 rounded-xl bg-zinc-950 border border-white/5 space-y-2">
                  <div className="flex items-center gap-2">
                    <span className="text-purple-400 font-mono font-bold">Q{idx + 1}.</span>
                    <input
                      type="text"
                      value={q.title}
                      onChange={(e) => handleQuestionChange(q.id, 'title', e.target.value)}
                      className="flex-1 px-2.5 py-1.5 rounded-lg bg-zinc-900 border border-white/10 text-zinc-200 focus:outline-none"
                    />
                    <select
                      value={q.type}
                      onChange={(e) => handleQuestionChange(q.id, 'type', e.target.value)}
                      className="px-2 py-1.5 rounded-lg bg-zinc-900 border border-white/10 text-zinc-300 focus:outline-none text-[11px]"
                    >
                    ...
                    <div className="pl-6 space-y-1.5 pt-1">
                      {q.options?.map((opt, optIdx) => (
                        <div key={optIdx} className="flex items-center gap-2">
                          <span className="w-2.5 h-2.5 rounded-full border border-purple-400/50" />
                          <input
                            type="text"
                            value={opt}
                            onChange={(e) => handleOptionChange(q.id, optIdx, e.target.value)}
                            className="px-2 py-1 bg-zinc-900/80 border border-white/5 rounded text-zinc-300 text-[11px] focus:outline-none"
                          />
                        </div>
                      ))}
                      <button
            ...
            <button
              ...
              className="w-full mt-2 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white font-semibold transition-all flex items-center justify-center gap-2"
            >
              {isCreating ? (
                <>
            ...
            <div className="p-4 rounded-2xl bg-emerald-950/40 border border-emerald-500/30 space-y-2">
              ...
              {createdForm.webViewLink && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1.5 text-purple-400 hover:text-purple-300 font-medium underline pt-1"
                >
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-zinc-900 border border-white/10 text-zinc-300 hover:bg-zinc-800"
            >
          ...
          <div className="grid grid-cols-1 gap-2.5">
            {formsList.map((form) => (
              <div
                key={form.id}
                className="p-3.5 rounded-2xl bg-zinc-900/80 border border-white/5 hover:border-purple-500/30 transition-all flex items-center justify-between"
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <FileText className="w-4 h-4 text-purple-400" />
                    <span className="font-semibold text-zinc-100">{form.title}</span>
                  </div>
                  {form.description && (
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl bg-purple-600/20 text-purple-300 hover:bg-purple-600/30 font-medium border border-purple-500/30 transition-all"
                  >
                    Respond
                  </button>
                  {form.webViewLink && (
      ...
      {activeSubTab === 'respond' && (
        <div className="space-y-4">
          {selectedForm ? (
            <div className="p-4 rounded-2xl bg-zinc-900/80 border border-white/5 space-y-4">
              <div>
                <div className="text-xs text-purple-400 font-semibold mb-1">Active Form</div>
                <h3 className="text-sm font-bold text-zinc-100">{selectedForm.title}</h3>
              ...
              {submitSuccess ? (
                <div className="p-4 rounded-xl bg-emerald-950/40 border border-emerald-500/30 text-emerald-300 flex items-center gap-2 font-medium">
                ...
                <div className="space-y-3 pt-2">
                    ...
                    <div key={q.id} className="p-3 rounded-xl bg-black/40 border border-white/5 space-y-2">
                      ...
                      {q.type === 'text' ? (
                        <input
                          ...
                          className="w-full px-3 py-1.5 rounded-lg bg-zinc-900 border border-white/10 text-zinc-200 focus:outline-none"
                        ...
                        <div className="space-y-1.5 pt-1">
                          {(q.options || []).map((opt) => (
                            <label
                              key={opt}
                              className="flex items-center gap-2 p-2 rounded-lg bg-zinc-900/50 hover:bg-zinc-800/50 cursor-pointer text-zinc-300"
                            >
                              <input
                                type="radio"
                                name={q.id}
                  ...
                  <button
                    ...
                    className="w-full py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white font-semibold transition-all flex items-center justify-center gap-2"
                  >
                    {isSubmitting ? (
                      <RefreshCw className="w-4 h-4 animate-spin" />
                    ) : (
