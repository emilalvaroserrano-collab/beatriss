...
export const GmailTool: React.FC<GmailToolProps> = ({ onEmailSent }) => {
  ...
  return (
    <div className="flex flex-col gap-3 text-xs text-zinc-100">
      ...
      <div className="flex items-center justify-between p-3 rounded-2xl bg-indigo-950/40 border border-indigo-500/20">
        <div className="flex items-center gap-2.5">
          <div className="p-2 rounded-xl bg-indigo-500/20 text-indigo-300">
            <Mail className="w-4 h-4" />
          </div>
          <div>
            <div className="font-semibold text-indigo-200">Gmail API Integration</div>
        ...
        <div className="flex items-center gap-2">
          {!accessToken && (
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-medium transition-all text-[11px]"
            >
              Authorize Gmail
            </button>
          )}
      ...
      <div className="flex items-center justify-between border-b border-white/5 pb-2">
        ...
        {activeTab === 'inbox' && !selectedMessage && (
          <button
            ...
            disabled={isLoading}
            className="flex items-center gap-1 px-2.5 py-1 rounded-xl bg-zinc-900 border border-white/10 text-zinc-300 hover:bg-zinc-800 transition-all text-[11px]"
          >
      ...
      {activeTab === 'inbox' && !selectedMessage && (
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 bg-zinc-900/90 p-1.5 rounded-xl border border-white/10">
            ...
            <button
              onClick={() => handleFetchMessages(searchQuery)}
              className="px-2.5 py-1 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-[11px] font-medium"
            >
              Search
            </button>
          </div>
            ...
            {isLoading ? (
              ...
              messages.map((msg) => (
                ...
                >
                  ...
                  <div className="font-medium text-zinc-100 line-clamp-1 mb-1">{msg.subject}</div>
      ...
      {activeTab === 'inbox' && selectedMessage && (
        <div className="p-4 rounded-2xl bg-zinc-900/90 border border-white/10 space-y-3">
          ...
          <div className="border-b border-white/10 pb-3">
            <h3 className="text-sm font-bold text-zinc-100 mb-2">{selectedMessage.subject}</h3>
          ...
          <div className="flex items-center gap-2 pt-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold transition-all"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-purple-600/20 text-purple-300 hover:bg-purple-600/30 border border-purple-500/30 transition-all font-medium"
            >
          ...
          <div className="p-3 rounded-xl bg-indigo-950/50 border border-indigo-500/30 space-y-2">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 px-3 py-1.5 rounded-xl bg-black/60 border border-white/10 text-zinc-200 text-xs focus:outline-none focus:border-indigo-500"
              />
              <button
                onClick={handleGenerateAiEmail}
                disabled={isAiGenerating || !aiPrompt.trim()}
                className="px-3 py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-medium disabled:opacity-50 transition-all flex items-center gap-1 whitespace-nowrap"
              >
          ...
          <div className="space-y-2.5">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-1.5 rounded-xl bg-black/50 border border-white/10 text-zinc-100 text-xs focus:outline-none focus:border-indigo-500"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3 py-2 rounded-xl bg-black/50 border border-white/10 text-zinc-100 text-xs focus:outline-none focus:border-indigo-500 resize-none font-sans"
              />
            </div>
            ...
            <button
              ...
              className="w-full py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-semibold transition-all flex items-center justify-center gap-2 text-xs"
            >
              {isSending ? (
                <>
