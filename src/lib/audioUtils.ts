...
export class AudioController {
  ...
  /**
   * Returns real-time 32-bin frequency data and waveforms for mic input & AI output
   */
  public getInputSpectrum(): Uint8Array {
    if (!this.analyserInput || this.isMuted) {
  ...
  public getInputWaveform(): Uint8Array {
    if (!this.analyserInput || this.isMuted) {
      return new Uint8Array(64);
    }
    const data = new Uint8Array(64);
    this.analyserInput.getByteTimeDomainData(data);
    return data;
  }

  public getRealtimeSpectrum(): {
    inputFreqs: number[];
    outputFreqs: number[];
    inputVol: number;
    outputVol: number;
  } {
    const inArray = new Uint8Array(32);
    const outArray = new Uint8Array(32);

    if (this.analyserInput && !this.isMuted) {
      this.analyserInput.getByteFrequencyData(inArray);
    }
    if (this.analyserOutput && this.hasActivePlayback()) {
      this.analyserOutput.getByteFrequencyData(outArray);
    }

    const inputFreqs = Array.from(inArray).map((v) => v / 255);
    const outputFreqs = Array.from(outArray).map((v) => v / 255);

    const inputVol = inputFreqs.length
      ? inputFreqs.reduce((a, b) => a + b, 0) / inputFreqs.length
      : 0;
    const outputVol = outputFreqs.length
      ? outputFreqs.reduce((a, b) => a + b, 0) / outputFreqs.length
      : 0;

    return { inputFreqs, outputFreqs, inputVol, outputVol };
  }

  public getWaveformData(): { inputWave: Uint8Array; outputWave: Uint8Array } {
    const inWave = new Uint8Array(64);
    const outWave = new Uint8Array(64);

    if (this.analyserInput && !this.isMuted) {
      this.analyserInput.getByteTimeDomainData(inWave);
    }
    if (this.analyserOutput && this.hasActivePlayback()) {
      this.analyserOutput.getByteTimeDomainData(outWave);
    }

    return { inputWave: inWave, outputWave: outWave };
  }
